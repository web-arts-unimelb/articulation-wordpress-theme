...
function twentyten_setup() {
	...
	// Add a filter to twentyten_header_image_width and twentyten_header_image_height to change these values.
 	//SL changed the height.
	define( 'HEADER_IMAGE_WIDTH', apply_filters( 'twentyten_header_image_width', 940 ) );
	define( 'HEADER_IMAGE_HEIGHT', apply_filters( 'twentyten_header_image_height', 137 ) ); //SL changed the height

	// We'll be using post thumbnails for custom header images on posts and pages.
	// We want them to be 940 pixels wide by 137 pixels tall.
...
// Control what is on home page
add_filter('pre_get_posts', 'my_home_get_posts');

// 
add_filter('the_content', 'my_home_the_content');


// --------------------------------------------------------------- Define your functions here
function __translate_page_id_to_entity_name($page_id=null)
{
    $entity_name = null;
 
    // page_id is generated by wordpress   
    if($page_id == "1579")
    {
        $entity_name = "ai";
    }
    ...
    {
				// Melbourne school of government
				// In dev and gary's local. It is different from production site
        $entity_name = "msog";
    }
    elseif($page_id == "1649")
    {
...
function __translate_entity_name_to_category_id($entity_name=null)
{
    $category_id = null;
    
    // category id is generated by wordpress
    if($entity_name == "ai")        
    {
        $category_id = "3";        
    }
    if($entity_name == "gshss")        
    {
        $category_id = "7";        
    }
    if($entity_name == "shaps")        
    {
        $category_id = "11";        
    }
    if($entity_name == "scc")        
    ...
    if($entity_name == "ssps")        
    {
		...
		if($entity_name == "msog")
    {
				// This is same in production, dev and gary's local
        $category_id = "26";
    }
...
function _get_post_ids_by_event_time($category_array, $num_of_posts)
{
  ...
  $posts = get_posts($args);
	...
	foreach($post_array as $post_id => $event_time)
	{
		...
		$return_array[$post_id] = $event_time;
		++$count;
	}
...
// Start feeds ----------------------------
function create_myfeeds() 
{
	// You'll create a your-custom-feed.php file in your theme's directory
	load_template(TEMPLATEPATH. '/myfeeds.php');	
}
// Make sure to have 'do_feed_customfeed'
add_action('do_feed_myfeeds', 'create_myfeeds', 10, 1); 
// End feeds ----------------------------


// Category posts by years
// Parse the event time
function __get_year($event_time)
{
	$return = null;

	// $event_time format is 2014-04-04 e.g. 
	if(empty($event_time))
	{
		$return = "Other";
	}
...
function _is_gshss_category($category_id)
{
	$gshss_category_id = 7;
	if($category_id == $gshss_category_id)
	{
		return true;
	}
	else
...
function _get_current_template_directory_name()
{
...
function _build_current_theme_url()
{
	$theme_name = _get_current_template_directory_name();
	$url = site_url(). "/wp-content/themes/". $theme_name;
	return $url;
}

function _get_gshss_banner_url()
{
	$url = _build_current_theme_url(). "/images/misc/gshss_logo.jpg";
	return $url;
}

function _display_entity_banner($category_id)
{
	$html = ""; 

	if(_is_gshss_category($category_id))
	{
		$image_url = _get_gshss_banner_url();
		$html = "
			<img src=\"$image_url\" />
		";
	}
...
function my_home_the_content($content) {
    ...
    $tmp_content = str_replace($needle, 'my_home_content_remove', $content);
		...
		$document->loadHTML($tmp_content);
		$rootElement = $document->documentElement;
		$unwantedTags = $rootElement->getElementsByTagName('p');

		foreach($unwantedTags as $tag)
		{
			if(preg_match('/^my_home_content_remove/', $tag->nodeValue)) {
